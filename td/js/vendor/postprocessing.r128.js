// three.js r128 후처리 묶음 — examples/js 에서 그대로 가져와 하나로 합쳤다(2026-09-07, M-4-1).
// 원본: https://unpkg.com/three@0.128.0/examples/js/{shaders,postprocessing}/*.js  (MIT, three.js 와 같은 라이선스)
// 합친 이유: index.html 의 SCRIPTS 목록과 로딩 횟수를 늘리지 않으려고. 내용은 한 글자도 고치지 않았다.
// 🛑 순서가 중요하다 — EffectComposer.js 안에 밑바탕 클래스(Pass·FullScreenQuad)가 들어 있어서
//    ShaderPass·MaskPass·RenderPass·UnrealBloomPass(전부 Pass 를 물려받는다)보다 **먼저** 와야 한다.
//    처음엔 이름 순서대로 놨다가 `extends undefined` 로 뒤쪽 파일이 통째로 죽었다(실측).
